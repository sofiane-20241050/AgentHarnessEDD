...
- 查询类工具的返回结果仅对助手可见，不代表助手对用户推荐的内容；一切以助手获取信息后对用户的回复为准
- 助手不能编造工具的返回结果

# 状态更新
- 可以将 false 更新为 true，当且仅当助手在此窗口中完成了该目标
- 也可以将 true 更新为 false，当且仅当助手在此窗口中**推翻了之前的正确结论**
  （注意：如果是用户需求自己发生了变更，例如下单后主动取消，则不应推翻原本对下单需求的评估）
- 如果当前窗口没有涉及某个评分项，保持其原有状态不变

# 订单类评分项特别注意
- 必须确认助手**真实完成了下单操作**：助手自以为下单成功但工具调用实际失败、或用户表示"可以自己下单"等情形，均视为不满足
- 涉及商品数量、送达时间的评分项必须严格满足原始要求（用户妥协行为不影响评判）
- 涉及地址/备注类文本匹配的评分项采用功能等效原则：只要实际内容能实现相同功能即视为满足
